...
public class GenericPropertySheet extends PropertySheet {
  ...
  public static class CustomItem implements PropertySheet.Item {
	...
	public CustomItem(final String key, final String name, final String group, final String regex, final Object value,
  ...
  public <T> void addItem(final String key, final String name, final String group, final String descr,
	  final String regex, final T value, EditorType inputType, String[] choices) {
  ...
  public <T> void addItem(final String key, final String name, final String group, final String descr,
	  final String regex, final T value, EditorType inputType) {
	final Optional<CustomItem> item = getItem(key);
	final CustomItem it;
	if (item.isPresent()) {
	  it = item.get();
  ...
  public Optional<CustomItem> getItem(final String key) {
	return this.getItems().stream().filter((it) -> key.equals(((CustomItem) it).getKey())).map(el -> (CustomItem) el)
		.findFirst();
  }
  ...
  public GenericPropertySheet() {
	this.setPropertyEditorFactory(param -> {
	  ...
	  if (value != null) {
		...
		} else if (item.inputType == EditorType.FILE) {
  ...
  /**
   * Will set the textFields text-fill red if invalid, green if valid or black if
   * empty.
   * 
   * @param regex
   *          Regular expression the input has to match.
   * @param target
   *          The textField to add validation to.
   */
  public static void addValidator(final String regex, final TextField target) {
	final Pattern p = Pattern.compile(regex);
	EventStreams.valuesOf(target.textProperty()).successionEnds(Duration.ofMillis(250)).subscribe((value) -> {
   ...
   */
  public static final Class<?>[] NumericTypes = new Class[] { byte.class, Byte.class, short.class, Short.class,
   ...
   * @param type
   *          A type to check.
   * @return <code>true</code> if and only if the given type is a default numeric
   *         type.
   */
  ...
  public static final PropertyEditor<?> createFileEditor(Item property) {

	HBox control = new HBox();
	TextField input = new TextField();
	Button button = new Button("Datei wählen...");
	Stage stage = new Stage();
	FileChooser chooser = new FileChooser();

	ObjectProperty<File> prop = new SimpleObjectProperty<>();
	input.textProperty().bindBidirectional(prop, new StringConverter<File>() {
	  @Override
	  public File fromString(String f) {
		return (f == null) ? null : new File(f);
	  }

	  @Override
	  public String toString(File f) {
		return (f != null) ? f.toPath().toString() : "";
	  }
	});
	...
	control.getChildren().addAll(input, button);

	HBox.setHgrow(button, Priority.ALWAYS);
	HBox.setHgrow(input, Priority.ALWAYS);

	return new AbstractPropertyEditor<String, HBox>(property, control) {
